...
public class Train06_01_5InsertionSort {
 ...
 static void insertionSort(int[] a, int n) {//코드를 이해하여 알고리즘을 파악한다.
     ...
     for (int i = 1; i < n; i++) {
         int j;
         int tmp = a[i];//a[i]가 선택한 요소 -카드를 하나 꺼내서 //다음 차례 카드
         //선택한 카드의 좌측에 넣을 곳을 찾는다 - 이미 정렬된 좌측 카드 순서 배열에 삽입
         for (j = i; j > 0 && a[j - 1] > tmp; j--) {//a[j-1]을 기준으로 거꾸로 이동
        	 count++;
        	 a[j] = a[j - 1];//이미 정렬된 부분에 tmp가 들어갈 자리를 찾는다 -뒤로 밀어낸다- 이미 정렬된 효과 덕분에 성능 향상 얻어짐
         }
         count++;
